...
# this are transforming functions that i have borrowed form rsa library
# credits: https://github.com/sybrenstuvel/python-rsa/blob/main/rsa/transform.py
...
class RSA:
    ...
    def cipher(self, text=None, file=None, out_file=None, public_key=None, block_size=1):
        ...
        if public_key is not None:
            ...
            key = self.public_key
        if block_size != 1:
            # also if block_size > 1 we should check if 2^(rozmiar_klucza) > rozmiar_zestawu_symboli ^(rozmiar_bloku)
            # eg for us : 2**self.n > len(symbols)**block_size
            assert 2 ** mod > len(self.SYMBOLS) ** block_size, "The key is too small :("
        if text is not None:
            res = self.__cipher_blocks(text, block_size, mod, key)
        elif file is not None:
            with open(file, 'r') as file_text:
                res = self.__cipher_blocks(file_text, block_size, mod, key)
    ...
    def decipher(self, text=None, file=None, out_file=None, block_size=1):
        res = None
        ...
        if block_size == 1:
            ...
            byte_arrays = byte_string.split(b';')
        print(byte_arrays)
        reconstructed_numbers = [bytes2int(b) for b in byte_arrays]
        # decipher
        res = self.__decipher_blocks(reconstructed_numbers, block_size)
    ...
    def __cipher_blocks(self, text, block_size, mod, key):
        # if we use block size > 1 we should check if all letters are in symbols or not decode those that are not in them
        # also if block_size > 1 we should check if 2^(rozmiar_klucza) > rozmiar_zestawu_symboli ^(rozmiar_bloku)
        # eg for us : 2**self.n > len(symbols)**block_size
        ## returns a list of big numbers [ .... ]
        text = text.encode('utf-8')
        res = []
        nadmiar = len(text) % block
        if nadmiar != 0:
            overflow = text[-nadmiar:]
            text = text[:-nadmiar]
            print(msg)
            print(overflow)
        for i in range(0, len(text), block_size):
            ...
            if block_size == 1:
                ...
                for j in range(block_size):
                    # character += self.symbols_dict[text[j]] * (len(self.SYMBOLS) ** j)
                    try:
                        val = self.symbols_dict[text[i + j]]
                        character += val * (len(self.SYMBOLS) ** j)
                    except Exception as e:
                        print(f'error: {e} for "{chr(text[i + j])}" : {text[j]}')
                print(character)
            ciphered = fast_modulo(character, key, mod)  # (character ** public_key ) % mod ;; mod = n = p*q
            res.append(ciphered)
        return res

    def __decipher_blocks(self, ciphered_numbers, block_size):  # <--------------------------------- here errors
        # gets a list of numbers returns text ? (at least for block size = 1)
        mod = self.p * self.q
        ...
        for i in range(0, len(ciphered_numbers)):
            ...
            if block_size == 1:
                ...
                characters = []
                for j in range(block_size - 1, -1, -1):  # eg for block_size = 4: 3 -> 2 -> 1 -> 0
                    # ----- string index out of range
                    # ----- were incorrectly fetching back the numbers - it's not the same - and then we have error
                    # because of SYMBOLS = [A:Za:z0:9] = out of range
                    temp = deciphered // (len(self.SYMBOLS) ** j)
                    print(temp)
                    characters.append(self.SYMBOLS[temp])
                # characters = characters[::-1] # reverse
                for letter in characters[::-1]:
                    res.append(letter)
        return "".join(res)
